...
export const AddIncome: React.FC<AddIncomeProps> = ({
  ...
  const [backButton, setbackButton] = useState('income');
  const {saveUser, user} = useUser();
  const [cashValueIncome, setCashValueIncome] = useState('');
  const [targetValue, setTargetValue] = useState('');

  // useEffect(() => {
  //   // Оновлюємо значення тільки при першому завантаженні, якщо воно є
  //   if (cashValueIncome) {
  //     setCashValueIncome(user?.totalIncome || '0');
  //   }
  // }, [cashValueIncome]);
  ...
  const handleNextPress = () => {
    if (!cashValueIncome || !targetValue || selectedElement === null) {
      Alert.alert(
        'Message',
        ...
        <SafeAreaView style={styles.main}>
          <SafeAreaView style={styles.input_container}>
            ...
            <View style={{gap: 5}}>
              ...
              <TextInput
                style={styles.input}
                value={cashValueIncome}
                onChangeText={setCashValueIncome}
                placeholder={'$'}
                placeholderTextColor={'rgba(255, 255, 255, 0.25)'}
              />
            </View>
